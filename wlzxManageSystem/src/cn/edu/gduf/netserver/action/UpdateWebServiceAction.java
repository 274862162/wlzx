...
public class UpdateWebServiceAction implements Action {
	...
	public String execute(HttpServletRequest request,
		...
		System.out.println("更新网络服务文件！");
		//文件ID
		int fileID = 0;
		// 工厂
		DiskFileItemFactory factory = new DiskFileItemFactory(20*1024,new File(request.getSession().getServletContext().getRealPath("files/webService/temp")));
		// 解析器
		...
		try {
			...
			String filePath = request.getSession().getServletContext().getRealPath("files/webService/");
			...
			if(list.get(2).isFormField()){
				if(list.get(2).getFieldName().equals("fileID")){
					...
					if(!(temp==null||temp.equals(""))){
						...
						System.out.println("啦啦啦,我是="+fileID);
					}
				}
			}
			WebServiceDao webServiceDao = new WebServiceDao();
			WebService webService = new WebService();
			webService.setFileID(fileID);
			webService.setFileName(fileName);
			webService.setFilePath(filePath);
			int isUpload = webServiceDao.updateWebService(webService);
			if(isUpload==0){
		...
		return "jsp/superManagement7.jsp";
	}
