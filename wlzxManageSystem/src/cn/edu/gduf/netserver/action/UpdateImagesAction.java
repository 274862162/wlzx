...
public class UpdateImagesAction implements Action {
	...
	public String execute(HttpServletRequest request,
		...
		int imagesID = 0;
		// 工厂
		DiskFileItemFactory factory = new DiskFileItemFactory(20*1024,new File(request.getSession().getServletContext().getRealPath("files/images/temp")));
		// 解析器
		...
		try {
			...
			String filePath = request.getSession().getServletContext().getRealPath("files/images/");
			...
			if(list.get(2).isFormField()){
				if(list.get(2).getFieldName().equals("imagesID")){
					String temp = list.get(2).getString("utf-8");
					if(!(temp==null||temp.equals(""))){
						imagesID= Integer.parseInt(temp);
					}
				}
			}
			ImagesDao imagesDao = new ImagesDao();
			Images images = new Images();
			images.setImagesID(imagesID);
			images.setFileName(fileName);
			images.setFilePath(filePath);
			int isUpload = imagesDao.updateImages(images);
			if(isUpload==0){
