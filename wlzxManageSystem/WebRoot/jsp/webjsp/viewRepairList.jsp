...
<body>
	...
	<table class="w980 margin0_auto f14">
		<c:forEach items="${sessionScope.repairLists}" var="userRrepairList">
			<c:if test="${userRrepairList.repairListID==sessionScope.displayID}">
				<tr><td colspan="4" class="repairMessage">报修信息</td></tr>
				<tr>
				<td class="leftTd"><span class="mandatory">*</span>学号：</td><td><input type="text" class="leftTdText" value="${userRrepairList.stuSno}" disabled/></td><td colspan="2" class="rightTd"><span class="mandatory">*</span>姓名：<input type="text" class="rightTdText nameText" value="${userRrepairList.stuName}" disabled/>
				处理时间：<input type="text" value="${userRrepairList.dealingTime}" disabled/>
				</td></tr>
				<tr>
					...
					<span class="remarks">预约备注：<input type="text" id="appointmentText" value="${userRrepairList.appointmentText}" disabled/></span></td>
				</tr>
				...
				处理网管：
				<input type="text" value="${userRrepairList.dutyPersonID}" disabled/>
				</td>				
				</tr>
				...
				<td colspan="3">
					<textarea class="text_indent_2em" cols="134" rows="15" name="problemDescribe" id="problemDescribe" onblur="descriptionLimit()">
						<c:out value="${userRrepairList.dealingText}"></c:out>
					</textarea>
				</td>
				</tr>
			</c:if>
    ...
    <div class="repairs processing button">
    	<input type="button" value="返回" id="cancel"/>
	</div>
	<!--按钮结束-->
</body>
