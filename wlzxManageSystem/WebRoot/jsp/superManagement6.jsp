...
<head>
	...
	<script type="text/javascript">
		...
		function show(fID){
			...
			param.fileID = fID;
			$.ajax({
		 	   url  : "CommonServlet_ajax?action=getLearningFile",  
		 	   data : param,
		 	   type : "post",                  
		 	   cache: false,
		 	   dataType:"json", 
		...
		function del(fID){
			...
			param.fileID = fID;
			$.ajax({
		 	   url  : "CommonServlet_ajax?action=delLearningFile",  
		 	   data : param,
		 	   type : "post",                  
		 	   cache: false,
		 	   dataType:"text", 
	 		   ...
	 		   success :function(result){  //请求成功时调用。 
	 		   		alert(result);
					window.location.href="jsp/superManagement6.jsp";		 			 
	 		   }
 			});
		};
		//取学习天地文件列表
		function getLearningFiles(){
			...
			$.ajax({
	 		   url  : "CommonServlet_ajax?action=learningShow",  
	 		   data : param,
	 		   type : "post",                  
	 		   cache: false,
	 		   dataType:"json", 
	 		   ...
	 		   success :function(result){  //请求成功时调用。 
	 		   		$("#tb_LearningFiles").children($("<tr>")).remove();
	 		   		$("#tb_LearningFiles").append('<tr class="head"><td>发布时间</td><td>学习文件</td><td>操作人</td><td>操作</td></tr>');
	 		   		$.each(result,function(){
		 			    $.each(this.list,function(){
		 			    	$("#tb_LearningFiles").append('\
...
<div class="wrapper margin0_auto" style="height:100%;">
        ...
        <div class="personInfo">
			<div class="personInfo_nav f14">超级管理&nbsp;&nbsp;>>&nbsp;&nbsp;前台管理</div>
             <ul class="none_list_style_type">
                ...
                <li class="f14 float_l"><a href="jsp/superManagement6.jsp" class="f_blue0066ff">学习天地管理</a></li>
            ...
            <div class="queryWage superManagement4">
            	<table id="tb_LearningFiles">
                    <tr class="head"><td>发布时间</td><td>学习文件</td><td>操作人</td><td>操作</td></tr>
                    <%-- <c:forEach var="ll" items="${learningList}" varStatus="status">
                    <tr><td><fmt:formatDate value="${ll.createTime}" type="date"/></td><td>${ll.fileName }</td><td>${ll.operator }</td><td><a href="SuperManagementServlet?action=delLearningFile&fileID=${ll.fileID }">删除</a>&frasl;<a href="javascript:show(${ll.fileID });" id="modifyButton">修改</a></td></tr>
                	</c:forEach> --%>
                </table>
                ...
                <div class="button" id="recommendedPlan-Button2"><input type="button" value="添加文件"></div>
            </div>
        </div>	
    	<!--个人信息end-->
    ...
    <div id="recommendedPlan">
        <div class="recommendedPlan-title">修改学习文件<img src="images/surveySystem/delete.png" title="close" alt="close" class="close" id="close-button" /></div>
        <div class="f14 recommendedPlan">
            <form name="superManagement6Form" method="post" action="CommonServlet_redirect?action=updateLearningFile" enctype="multipart/form-data">
    ...
    <div id="recommendedPlan2">
        <div class="recommendedPlan-title">添加学习文件<img src="images/surveySystem/delete.png" title="close" alt="close" class="close" id="close-button2" /></div>
        <div class="f14 recommendedPlan">
            <form name="superManagement6Form" method="post" action="CommonServlet_redirect?action=addLearningFile" enctype="multipart/form-data">
                <div class="queryWage superManagement4 superManagement7 superManagement8Modal">
                    <table>
