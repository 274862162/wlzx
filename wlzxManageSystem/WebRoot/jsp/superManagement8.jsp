...
<head>
	...
	<script type="text/javascript">
		...
		function show(fID){
			...
			param.fileID = fID;
			$.ajax({
		 	   url  : "CommonServlet_ajax?action=getVideo",  
		 	   data : param,
		 	   type : "post",                  
		 	   cache: false,
		 	   dataType:"json", 
		...
		function del(fID){
			...
			param.fileID = fID;
			$.ajax({
		 	   url  : "CommonServlet_ajax?action=delVideo",  
		 	   data : param,
		 	   type : "post",                  
		 	   cache: false,
		 	   dataType:"text", 
	 		   ...
	 		   success :function(result){  //请求成功时调用。 
	 		   		alert(result);
					window.location.href="jsp/superManagement8.jsp";		 			 
	 		   }
 			});
		};
		//取网络服务文件列表
		function getVideo(){
			...
			$.ajax({
	 		   url  : "CommonServlet_ajax?action=videoShow",  
	 		   data : param,
	 		   type : "post",                  
	 		   cache: false,
	 		   dataType:"json", 
	 		   ...
	 		   success :function(result){  //请求成功时调用。 
	 		   		$("#tb_video").children($("<tr>")).remove();
	 		   		$("#tb_video").append('<tr class="head"><td>发布时间</td><td>视频</td><td>操作人</td><td>操作</td></tr>');
...
<div class="wrapper margin0_auto" style="height:100%;">
	...
	<div class="content" style="height:100%;">
        ...
        <div class="personInfo">
            ...
            <ul class="none_list_style_type">
                ...
                <li class="f14 float_l"><a href="jsp/superManagement8.jsp" class="f_blue0066ff">视频管理</a></li>
            </ul>   
            <hr/>
            <div class="queryWage superManagement4">
            	<table id="tb_video">
                   <tr class="head"><td>发布时间</td><td>视频</td><td>操作人</td><td>操作</td></tr>
                   <%--  <c:forEach var="vl" items="${videoList}" varStatus="status">
                    <tr><td><fmt:formatDate value="${vl.createTime}" type="date"/></td><td>${vl.fileName }</td><td>${vl.operator }</td><td><a href="SuperManagementServlet?action=delVideo&fileID=${vl.fileID }">删除</a>&frasl;<a href="javascript:show(${vl.fileID });" id="modifyButton">修改</a></td></tr>
                	</c:forEach> --%>
                </table>
                ...
                <div class="button" id="recommendedPlan-Button2"><input type="button" value="添加视频"></div>
            </div>
        </div>	
    	<!--个人信息end-->
    ...
    <div id="recommendedPlan">
        <div class="recommendedPlan-title">添加视频文件<img src="images/surveySystem/delete.png" title="close" alt="close" class="close" id="close-button" /></div>
        <div class="f14 recommendedPlan">
            <form name="superManagement6Form" method="post" action="CommonServlet_redirect?action=updateVideo" enctype="multipart/form-data">
    ...
    <div id="recommendedPlan2">
        <div class="recommendedPlan-title">修改视频文件<img src="images/surveySystem/delete.png" title="close" alt="close" class="close" id="close-button2" /></div>
        <div class="f14 recommendedPlan">
            <form name="superManagement6Form" method="post" action="CommonServlet_redirect?action=addVideo" enctype="multipart/form-data">
                <div class="queryWage superManagement4 superManagement7 superManagement8Modal">
                    <table>
