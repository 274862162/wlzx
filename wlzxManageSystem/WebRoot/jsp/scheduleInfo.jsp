...
<head>
    ...
    <title>scheduleInfo-我的无课表</title>
                ...
                <table>
                    ...
                    <c:forEach var="ft" items="${freeTablesList}" varStatus="status">
                    	<tr>
                    		...
                    		<td>
                    			<a href="javascript:void(0);" onclick="scheduleShow(${currentUser.userID}, ${ft.freeTableID});" id="recommendedPlan-Button">查看</a>&nbsp;/&nbsp;
                  				<a href="ScheduleServlet?action=scheduleFillGo&freeTableID=${ft.freeTableID}&freeTableName=${ft.freeTableName}">填写</a>
                    			<!-- 
                    				<c:forEach var="uft" items="${userFreeTableList}">
                    				<c:choose>
									<c:when test="${ft.freeTableID == uft.freeTableID && currentUser.userID == uft.userID}">
										<a href="javascript:void(0);" onclick="scheduleShow(${currentUser.userID}, ${ft.freeTableID});" id="recommendedPlan-Button">查看</a>&nbsp;/&nbsp;
                  						<a href="ScheduleServlet?action=scheduleFillGo&freeTableID=${ft.freeTableID}&freeTableName=${ft.freeTableName}">重新填写</a>	
									</c:when>
									<c:otherwise>
										<a href="javascript:void(0);" onclick="scheduleShow(${currentUser.userID}, ${ft.freeTableID});" id="recommendedPlan-Button">查看</a>&nbsp;/&nbsp;
                  						<a href="ScheduleServlet?action=scheduleFillGo&freeTableID=${ft.freeTableID}&freeTableName=${ft.freeTableName}">填写</a>	
